...
from XStrangeRobot import BOT_USERNAME as bu
from XStrangeRobot import BOT_ID, pbot, arq
from XStrangeRobot.ex_plugins.chatbot import add_chat, get_session, remove_chat
from XStrangeRobot.utils.pluginhelper import admins_only, edit_or_reply
...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("XStrange AI Already Activated In This Chat")
            return
        await lel.edit(f"XStrange AI Actived by {message.from_user.mention()} for users in {message.chat.title}")

    elif status == "OFF" or status == "off" or status == "Off":
        lel = await edit_or_reply(message, "`Processing...`")
        Escobar = remove_chat(int(message.chat.id))
        if not Escobar:
            await lel.edit("XStrange AI Was Not Activated In This Chat")
            return
        await lel.edit(f"XStrange AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}")
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        test = msg
        test = test.replace("XStrange", "Aco")
        test = test.replace("XStrange", "Aco")
        ...
        test = test.replace("@machinaxsupport", "XStrangebot.ai")
        ...
        test = test.replace(
            "Hello there I am XStrange...nice to meet u",
            "Hi, my friend! Do you want me to tell you a joke?")
        test = test.replace("@saint_foire is my owner", "Have the control right.")
        test = test.replace(
            "Hi, My name is XStrange Nice to meet you.",
            "Hi, my friend, what can I do for you today?")
        response = await lunaQuery(
            test, message.from_user.id if message.from_user else 0
        )
        response = response.replace("Aco", "XStrange")
        response = response.replace("aco", "XStrange")
        response = response.replace("Luna", "XStrange")
        response = response.replace("luna", "XStrange")
        ...
        response = response.replace("XStrangebot.ai", "@machinaxsupport")
...
@pbot.on_message(filters.regex("XStrange|XStrange|robot|XSTRANGE|TheAnnyOP") & ~filters.bot & ~filters.via_bot  & ~filters.forwarded & ~filters.reply & ~filters.channel & ~filters.edited)
async def inuka(client, message):
    msg = message.text
    if msg.startswith("/") or msg.startswith("@"):
        message.continue_propagation()
